...
out = cv2.VideoWriter('challenge_output_Final.avi',
                      cv2.VideoWriter_fourcc('M','J','P','G'), 30, (1200,617))

# Undistorting the frame using the calibration and distance matrix
def undistort(frame_dist):
    '''
    Undistorting the frame using the calibration and distance matrix
    @frame_dist: distorted input frame
    '''
    ...
    correct = cv2.undistort(frame_dist, K, D, None, kNew)
...
def roc(coef_right, points_right_new):
    '''
    Calculating the radius of curvature
    @coef_right: distorted input frame
    @points_right: distorted input frame
    '''
    radius = ((1+((2*coef_right[0]*points_right_new[50][1])+coef_right[1])**2)**(3/2))/abs(2*coef_right[0])
    return radius

# Determining the homography and creating the warped image
def homography(warped_im):
    '''
    Determining the homography and creating the warped image
    @warped_im: warped frame - birds-eye-view
    '''
    ...
    warped_im = cv2.warpPerspective(warped_im, H, (200,500))
...
def unwarp(unwarped):
    '''
    Determining the inverse homography and creating the unwarped image
    @unwarped: warped image frame ready for unwarping
    '''
    source = np.array([[560, 50], [145, 220],[1180, 220], [725, 50]])#main
...
def direction(img_center, left_max, right_max, res_img, radius_dir):
    '''
    Determining the direction of the road using the image and the lane centers
    @img_center: center of the image
    @left_max: left column with max white pixels
    @right_max: left column with max white pixels
    @res_img: resultant image
    @radius_dir: radius
    '''
...
while True:
    ...
    if ret is True:
        ...
        radius_curve = roc(coeff_right, pnts_right)
